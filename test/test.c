...
int main(){
    ...
    db_close(&qq);
    Db dl; db_init(&dl);
    ASSERT(db_exec(&dl,"CREATE TABLE t (id INT PRIMARY KEY, name TEXT)",out,sizeof(out))==0,"delete create");
    ASSERT(db_exec(&dl,"INSERT INTO t VALUES (1, 'a')",out,sizeof(out))==0,"delete i1");
    ASSERT(db_exec(&dl,"INSERT INTO t VALUES (2, 'b')",out,sizeof(out))==0,"delete i2");
    ASSERT(db_exec(&dl,"INSERT INTO t VALUES (3, 'c')",out,sizeof(out))==0,"delete i3");
    ASSERT(db_exec(&dl,"DELETE FROM t WHERE id = 2",out,sizeof(out))==0,"delete one");
    db_exec(&dl,"SELECT * FROM t",out,sizeof(out));
    ASSERT(strstr(out,"| 1 ")!=0 && strstr(out,"| 3 ")!=0,"delete survivors");
    ASSERT(strstr(out,"| 2 ")==0,"delete gone");
    db_exec(&dl,"SELECT COUNT(*) FROM t",out,sizeof(out));
    ASSERT(strstr(out,"2")!=0,"delete count 2");
    ASSERT(db_exec(&dl,"DELETE FROM t WHERE id = 99",out,sizeof(out))==0,"delete no match ok");
    db_exec(&dl,"SELECT COUNT(*) FROM t",out,sizeof(out));
    ASSERT(strstr(out,"2")!=0,"delete no match count kept");
    ASSERT(db_exec(&dl,"DELETE FROM t",out,sizeof(out))==0,"delete all");
    db_exec(&dl,"SELECT COUNT(*) FROM t",out,sizeof(out));
    ASSERT(strstr(out,"0")!=0,"delete all count 0");
    ASSERT(db_exec(&dl,"INSERT INTO t VALUES (4, 'd')",out,sizeof(out))==0,"delete reinsert");
    db_exec(&dl,"SELECT * FROM t WHERE id = 4",out,sizeof(out));
    ASSERT(strstr(out,"d")!=0,"delete reinsert visible");
    db_close(&dl);
    ...
    ASSERT(btree_delete(&p2,&root,"k1",2)==0,"btree delete");
    ASSERT(btree_search(&p2,root,"k1",2,vb,&vl)!=0,"btree gone after delete");
    ASSERT(btree_delete(&p2,&root,"k1",2)!=0,"btree delete missing fails");
    ASSERT(btree_insert(&p2,&root,"k1",2,"v9",2)==0,"btree reinsert");
    ASSERT(btree_search(&p2,root,"k1",2,vb,&vl)==0 && vl==2 && memcmp(vb,"v9",2)==0,"btree reinsert value");
